...
import { Row, Col, Table, Tabs, Card } from 'antd';
...
export default class Surgery extends Component {
  ...
  render() {
    ...
    const { diffCategorySurgeryModule = [], diffLevelSurgeryModule = [] } = surgery;

    const diffCategorySurgeryData =
      diffCategorySurgeryModule && transformArr(diffCategorySurgeryModule);
        ...
        {rangeDateType === 'daily' && (
          <div className="autoHeightCardWrap">
            <div className="autoHeightCard" style={{ marginRight: 20, width: '50%' }}>
              ...
              <Tabs onChange={this.changeTypeTab} type="card" style={{ zIndex: 2 }}>
                {typeMap.map(data => (
            ...
            <div className="autoHeightCard" style={{ width: '50%' }}>
              ...
              <Tabs onChange={this.changeLevelTab} type="card" style={{ zIndex: 2 }}>
                {levelMap.map(data => (
                  <TabPane tab={data.name} key={data.key} />
                ))}
              </Tabs>
                ...
                chartSetting={{
                  scale: {
                    date: {
                      type: 'cat',
                      tickCount: Math.ceil(
                        diffLevelSurgeryData.length / this.switchTime(diffLevelSurgeryData.length)
                      ),
                      formatter: (text) => {
